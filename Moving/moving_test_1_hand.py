...
model = joblib.load("./1_hand/lgbm_model.pkl")
...
def vni_to_viet(text):
    unicode_to_vni = {
        ...
        'Y2': 'Ỳ', 'Y1': 'Ý', 'Y3': 'Ỷ', 'Y4': 'Ỹ', 'Y5': 'Ỵ', 'D9': 'Đ',
        "Ll": " Y TÁ "
    }
    
    vowels = "AEIOUYD"  # Thêm D vào để kiểm tra 'D9' cho Đ
    numbers = "123456789"
    i = 0
    new_text = []
    ...
    while running:
        ...
        if hands:
            ...
            if label != start:
                start = label
                starttime = time.time()
            if time.time() - motiontime >= 1:
                if len(xy) == 2 and label in moving:
                    move = ""
                    if nodes[test][0] + x - xy[0] > w * 2 / 3:
                        move = "r"
                    elif xy[0] - nodes[test][0] - x > w * 2 / 3:
                        move = "l"
                    if nodes[test][1] + y - xy[1] > h * 2 / 3:
                        move += "d"
                    elif xy[1] - nodes[test][1] - y > h * 2 / 3:
                        move += "u"
            ...
            if (time.time() - starttime >= 1):
                if (label == "[cách] ") and (maintext != []):
                    ...
                    move = ""
                if label + move != "":
                    if maintext != [] and maintext[-1][0] == label[0]:
                        maintext.pop(-1)
        ...
        detected_text_var.set("".join(maintext))
